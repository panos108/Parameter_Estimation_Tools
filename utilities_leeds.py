...
def objective(f, u, V_old, N_exp, nx, n_points, nu, theta, sigma, V, c1o, c2o):
    ...
    obj =-np.log(np.linalg.det(vv1)+0.0001)#-## np.linalg.eig(vv)[0][0]#
...
def give_data_from_exp(nu, nx, ntheta, N_exp, PC, date, file, info):
...
def compute_Hessian(f, X):
    ...
    for i in range(size):
        for j in range(size):
            ...
            X_c2 += epsilon* v[i, :] - epsilon * v[j, :]

            #                dmdx[i, j] =  (GP(X_left)[0].reshape((1,-1,)) -2 * GP(X_c)[0].reshape((1,-1,)) + GP(X_right)[0].reshape((1,-1,)))/(2/1e-4)**2
            dsdx[i, j] = (f(X_left).reshape((1, -1)) - f(X_c1).reshape((1, -1)) - f(X_c2).reshape((1, -1)) +
                          f(X_right).reshape((1, -1))) / (2 * epsilon) ** 2
    return dsdx


def objective_cov(f, u, x_meas, N_exp, nx, n_points, nu, V, c1o, c2o,theta):

    ntheta = np.shape(theta.reshape((1,-1)))[1]
    x_meas1 = np.zeros([N_exp, nx[0], n_points + 1])
    xmin = np.zeros(nx[0]-1)
    xmax = np.zeros(nx[0]-1)#-1)
    x_meas_norm = x_meas.copy()
    ...
    for k0 in range(N_exp):
        ...
        for i in range(n_points):
            F = integrator_model(f, nu, nx, ntheta, 'embedded', 'mope', dt[k0, i])
            Fk = F(x0=vertcat(x11), p=vertcat(u[k0, :], theta.reshape((-1,))))
